...
const Header = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const queryTerme = e.target.search.value;
    // console.log(queryTerme)
    e.target.reset();

    return navigate(`/search?q=${queryTerme}`)
  }
        ...
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          ...
          <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-search">
            ...
            <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
              {navThings.map((item) => (
                <li key={item.to}>
                  <NavLink to={item.to} className={({ isActive }) => isActive ? activeClass : inActiveClass} aria-current="page">{item.name}</NavLink>
                </li>
              ))}
            </ul>
          </div>
